...
@section('page-description', 'Buat dan kelola transaksi penjualan')

@section('content')
<div x-data="salesTransaction()" x-init="init()">
    <!-- Action Buttons -->
    ...
    <div class="grid grid-cols-12 gap-6">
        ...
        <div class="col-span-12 lg:col-span-8">
            ...
            <div class="bg-card-light dark:bg-card-dark p-6 rounded-lg shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" x-model="transaction.sale_date" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary focus:ring-primary sm:text-sm bg-white dark:bg-gray-700 text-text-light dark:text-text-dark"/>
                    </div>
                    <div class="relative">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Customer</label>
                        <div class="mt-1 flex rounded-md shadow-sm">
                            <input type="text" x-model="customerSearch" @input="searchCustomer()" @focus="showCustomerDropdown = true" placeholder="Ketik nama customer..." class="flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-l-md focus:ring-primary focus:border-primary sm:text-sm border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-text-light dark:text-text-dark"/>
                            <button @click="openAddCustomerModal()" class="inline-flex items-center px-3 rounded-r-md border border-l-0 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-sm hover:bg-gray-100 dark:hover:bg-gray-600">
                                <span class="material-icons text-sm">person_add</span>
                            </button>
                        </div>
                        <!-- Customer Dropdown -->
                        <div x-show="showCustomerDropdown && filteredCustomers.length > 0" @click.outside="showCustomerDropdown = false" class="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                            <template x-for="customer in filteredCustomers" :key="customer.id">
                                <div @click="selectCustomer(customer)" class="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <div class="flex items-center">
                                        <span class="font-normal block truncate" x-text="customer.name"></span>
                                    </div>
                                    <span class="text-gray-500 dark:text-gray-400 text-xs block" x-text="customer.phone"></span>
                                </div>
                            </template>
                        </div>
                    </div>
                                    ...
                                    <div @click="addItemToCart(product)" class="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100 dark:hover:bg-gray-700">
                                        <div class="flex justify-between">
                                            ...
                                            <div class="text-right">
                                                <span class="text-primary font-semibold" x-text="formatCurrency(product.selling_price)"></span>
            ...
            <div class="bg-card-light dark:bg-card-dark p-6 rounded-lg shadow-sm mt-6">
                <h2 class="text-lg font-semibold text-text-light dark:text-text-dark mb-4">Daftar Item Dijual</h2>
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <input type="number" x-model.number="item.quantity" @input="updateItemTotal(index)" min="1" :max="item.max_stock" class="w-20 rounded-md border-gray-300 dark:border-gray-600 shadow-sm bg-white dark:bg-gray-700 text-center text-sm"/>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
        ...
        <div class="col-span-12 lg:col-span-4">
            <div class="bg-card-light dark:bg-card-dark p-6 rounded-lg shadow-sm sticky top-4">
                ...
                <div class="space-y-3">
                    ...
                    <!-- Cash Fields -->
                    <div x-show="payment.method === 'cash'" class="space-y-3 mt-3">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jumlah Dibayar</label>
                            <input type="number" x-model.number="payment.paid_amount" @input="calculateChange()" min="0" step="1000" placeholder="0" class="block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary focus:ring-primary sm:text-sm bg-white dark:bg-gray-700 text-text-light dark:text-text-dark"/>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Kembalian</label>
                            <input type="text" :value="formatCurrency(payment.change)" class="block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm sm:text-sm bg-gray-50 dark:bg-gray-800 text-text-light dark:text-text-dark" disabled/>
                        </div>
                    </div>
    ...
    <div x-show="showSuccessModal" @keydown.escape.window="showSuccessModal = false" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center" x-cloak>
        ...
        <div @click.stop class="bg-card-light dark:bg-card-dark rounded-lg shadow-lg w-full max-w-md mx-auto p-6 z-50 text-center">
            ...
            <p class="text-gray-600 dark:text-gray-300 mb-6">Transaksi penjualan telah berhasil dibuat.</p>
            <div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-4 space-y-3 text-left mb-6">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold text-text-light dark:text-text-dark" x-text="formatDate(savedTransaction.sale_date)"></span>
                </div>
            </div>
            <div class="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-3">
                ...
                <a :href="`/sales/${savedTransaction.sale_id}`" class="w-full flex items-center justify-center py-2 px-4 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-card-light dark:bg-card-dark hover:bg-gray-50 dark:hover:bg-gray-600">
    ...
    <!-- Add Customer Modal -->
    <div x-show="showAddCustomerModal" @keydown.escape.window="showAddCustomerModal = false" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center" x-cloak>
        <div @click="showAddCustomerModal = false" class="fixed inset-0 bg-gray-900 bg-opacity-50"></div>
        <div @click.stop class="bg-card-light dark:bg-card-dark rounded-lg shadow-lg w-full max-w-md mx-auto p-6 z-50">
            <div class="flex justify-between items-center pb-3 border-b border-border-light dark:border-border-dark mb-4">
                <h2 class="text-xl font-bold text-text-light dark:text-text-dark">Tambah Customer Baru</h2>
                <button @click="showAddCustomerModal = false" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" x-model="newCustomer.name" class="block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary focus:ring-primary sm:text-sm bg-white dark:bg-gray-700 text-text-light dark:text-text-dark"/>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">No. Telepon</label>
                    <input type="text" x-model="newCustomer.phone" class="block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary focus:ring-primary sm:text-sm bg-white dark:bg-gray-700 text-text-light dark:text-text-dark"/>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Email (Opsional)</label>
                    <input type="email" x-model="newCustomer.email" class="block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary focus:ring-primary sm:text-sm bg-white dark:bg-gray-700 text-text-light dark:text-text-dark"/>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Alamat (Opsional)</label>
                    <textarea x-model="newCustomer.address" rows="2" class="block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary focus:ring-primary sm:text-sm bg-white dark:bg-gray-700 text-text-light dark:text-text-dark"></textarea>
                </div>
            </div>
            <div class="flex justify-end space-x-3 mt-6">
                <button @click="showAddCustomerModal = false" class="px-4 py-2 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500">
                    Batal
                </button>
                <button @click="saveNewCustomer()" class="px-4 py-2 rounded-md text-sm font-medium text-white bg-primary hover:bg-green-600">
                    Simpan Customer
                </button>
            </div>
        </div>
...
function salesTransaction() {
    return {
        // Data
        transaction: {
            invoice_number: '{{ $invoiceNumber ?? "PJL-" . date("Ymd") . "-001" }}',
        ...
        init() {
            console.log('Initializing sales transaction...');
        ...
        searchCustomer() {
            console.log('Searching customer:', this.customerSearch);
        ...
        selectCustomer(customer) {
            ...
            console.log('Customer selected, ID:', this.transaction.customer_id);
        },
        ...
        updateItemTotal(index) {
            ...
            if (item.quantity > item.max_stock) {
                alert('Jumlah melebihi stok tersedia! Stok: ' + item.max_stock);
        ...
        calculateChange() {
            if (this.payment.method === 'cash') {
                this.payment.change = this.payment.paid_amount - this.summary.total;
            }
        },
        ...
        canProcess() {
            ...
            if (this.payment.method === 'cash') {
                if (this.payment.paid_amount < this.summary.total) {
        ...
        async processTransaction() {
            ...
            if (!confirm('Proses transaksi penjualan ini?')) {
                return;
            }

            const formData = {
                customer_id: this.transaction.customer_id,
                sale_date: this.transaction.sale_date,
                items: this.items,
            ...
            if (this.payment.method === 'cash') {
                formData.paid_amount = this.payment.paid_amount;
                formData.change = this.payment.change;
        ...
        resetForm() {
            if (confirm('Reset form transaksi? Semua data akan hilang.')) {
                this.items = [];
                this.transaction.customer_id = null;
                this.customerSearch = '';
                this.productSearch = '';
                this.summary = {
                    subtotal: 0,
        ...
        async saveNewCustomer() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    this.selectCustomer(result.data);
